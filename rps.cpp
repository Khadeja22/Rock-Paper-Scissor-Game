...
void rps() {
    ...
    string gameWinner;
    int gameNum = getMenuChoice();
    cout << endl;
    while (gameNum != 3) {
        gameWinner = doGame(name1, name2, gameNum);
        announceWinner(gameWinner);
        cout << endl;
        gameNum = getMenuChoice();
        cout << endl;
    }
...
string getName(int playerNumber) {
    ...
    if (name.empty()) {
        ...
        //if empty name is given, invalid input, print error message 1
        printErrorMessage(1);
   
        //returning a default name since input was empty
        if (playerNumber == 1) {
            return "Rocky";
        }
...
bool isMoveGood(char move) {
    ...
    if ((move == 'R') || (move == 'r') || (move == 'P') || 
        (move == 'p') || (move == 'S') || (move == 's')) {
        return true;
    }
    return false;
...
int doRound(string p1Name, string p2Name) {
    ...
    char player1Move = getMove(p1Name);
    char player2Move = getMove(p2Name);

    if (isRoundWinner(player1Move, player2Move)) {
        return 1;
    }
    else if (isRoundWinner(player2Move, player1Move)) {
        return 2;
    }
